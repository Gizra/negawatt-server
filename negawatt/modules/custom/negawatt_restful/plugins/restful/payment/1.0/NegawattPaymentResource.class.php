...
/**
 * @file
 * Contains NegawattPaymentResource.
 */

class NegawattPaymentResource extends \RestfulDataProviderDbQuery implements \RestfulDataProviderDbQueryInterface {
  ...
  public function publicFieldsInfo() {
    ...
    $public_fields['frequency'] = array(
      ...
      'column_for_query' => 'negawatt_payment_normalized.frequency',
    );
  ...
  protected function handleSiteCategoryFilter($query, &$filter) {
    ...
    if (empty($taxonomy_array)) {
      ...
      // Modify the query to sum payment according to the meters in the category.
      $query->condition('negawatt_payment_normalized.meter_nid', $meters, 'IN');
      $query->fields('negawatt_payment_normalized', array('meter_nid'));
      $query->groupBy('negawatt_payment_normalized.meter_nid');
    }
    else {
      ...
      // Modify the query to sum payment in each of the sub categories.
  ...
  /**
   * Calculate min and max timestamp for payment query.
   *
   * Build a query with all the filters and joins, like in prepareSummary().
   * Add calculation of the min and max timestamp of payment data for the
   * account and frequency.
   *
   * @return array
   *  min and max timestamps for payment.
   */
   ...
   * @return mixed
   *  An array from meter ids to their payment consumption.
   */
   ...
   * @return mixed
   *  An array from category ids to their payment consumption.
   */
   ...
   *
   * Called when total-for-categories found only one category with payment.
   * Look for the payment consumption of the category's sites and prepare
   * a total section.
   *
   * @param $cat_id
   ...
   *
   * @return mixed
   *  An array from site ids to their payment consumption.
   */
  protected function prepareTotalForSites($cat_id, $sites, $meter_account) {
    // Query the sites that relate to this category id, and sum their payment.
    ...
    // Modify the query to sum payment according to the sites.
   ...
   *
   * Prepare a list of sub-categories and their total payment (kWh)
  ...
  protected function prepareSummary() {
    ...
    if (!empty($cat_result['summary'])) {
      ...
      $this->valueMetadata['payment']['summary'] = $cat_result['summary'];
      return;
    }
    ...
    $this->valueMetadata['payment']['summary'] = $summary;
  }
  ...
  protected function prepareNormalizationFactors() {
    ...
    // Pass info to the formatter
    $this->valueMetadata['payment']['normalization_factors'] = $normalization_factors_metadata;
  }
  ...
  public function getQuery() {
    ...
    // Make summary fields
    $query->addExpression('SUM(sum_amount)', 'sum_amount');
    ...
    $query->groupBy('payment_type');
    $query->groupBy('negawatt_payment_normalized.frequency');
  ...
  public function getQueryForList() {
    ...
    if (!empty($request['nodata'])) {
      ...
      // leaving only summary. Since there's no payment data with timestamp=0,
   ...
   *
   * Modify the filter for site-category to catch all the payment
