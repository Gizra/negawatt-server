...
/**
 * @file
 * Contains \NegawattTaxonomyTermMeterSite.
 */

class NegawattTaxonomyTermMeterSite extends \RestfulEntityBaseTaxonomyTerm {
  ...
  /**
   *  Get the children of the current site item by id.
   */
  ...
  public function process($path = '', array $request = array(), $method = \RestfulInterface::GET, $check_rate_limit = TRUE) {
    ...
    // Get the meter-site taxonomy
    $found = FALSE;
    foreach ($og_vocabs as $og_vocab) {
      $vocabulary = taxonomy_vocabulary_load($og_vocab->vid);
      if (strpos($vocabulary->machine_name, 'meter_site_') === FALSE) {
        // Not a meter-site vocabulary, skip.
        continue;
      }
    ...
    if (!$found) {
      throw new \RestfulBadRequestException('No meter-site vocabulary was found.');
    }
  ...
  protected function electricityMinMax($wrapper) {
    ...
    // First, list all children categories of the site.
    ...
    foreach($categories as $site) {
      $meters = array_merge($meters, taxonomy_select_nodes($site));
    }
